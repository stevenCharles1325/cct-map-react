        ...
        <div className="settings">
            ...
            <div className="settings-inp-frame d-flex justify-content-end align-items-center">
                <div className="settings-inp-box d-flex flex-column justify-content-between align-items-center">
                    ...
                    { createInputField({ id: "set-username", title: "Username:", type: "text" })}
                    { createInputField({ id: "set-password", title: "Password:", type: "text", btn: createPeekButton("pass-peek") })}
                    { createInputField({ id: "set-cPass", title: "Confirm-password:", type: "text", btn: createPeekButton("cPass-peek") })}
                    { createInputField({ id: "set-email", title: "Email:", type: "email" })}
                    { createInputField({ id: "set-number", title: "Number:", type: "text" })}

                    <div className="settings-btn-box d-flex flex-row justify-content-around align-items-center">
                        { produceBtn([{id: "settings-save", title: "Save"}, {id: "settings-reset", title: "Reset"}]) }
                    </div>
...
function createInputField({ id, type, title, btn }){
    ...
    return (
        <div key={id.concat("settings")} className=`settings-inp-cont d-flex justify-content-center align-items-center`>
            ...
            <input id=`${id}` className="inp-bar" type=`${type}`/>
            { btn ? btn : null }
        </div>
    );
}

function createPeekButton( classname ){
    return <div key={classname} className=`${classname} msg-container`> peek </div>
}
...
function produceBtn( ids ){
    return ids.map( id => (<button id={id.id} className="settings-btn">{ id.title }</button>) );
}
