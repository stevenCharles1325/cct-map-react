...
export default class Signup extends React.Component{
    constructor( props ){
        ...
        this.mountInputMonitor = setInterval(() => { 
            let countReady = 0;

            for( let key of Object.keys( this.state ) ){
                if( this.state[key].isReady ){
                    countReady += 1;
                }
            }
            if( document.querySelector('#signup-submit') ){
                document.querySelector('#signup-submit').disabled = countReady === 4 ? false : true;
            }
        }, 1000)

        this.unmountInputMonitor = this.unmountInputMonitor.bind( this );
    }

    unmountInputMonitor() {
        clearInterval( this.mountInputMonitor );
    }
    ...
    updateState( name, pass, email, cnum ) {
        this.setState({
            ...
            username: { isReady: this.state.username.isReady, value: name.value},
            password: { isReady: this.state.password.isReady, value: pass.value},
            email: { isReady: this.state.email.isReady, value: email.value},
            number: { isReady: this.state.number.isReady, value: cnum.value}
        })
    }
                ...
                <div className="sign-up-frame d-flex flex-row">
                    <div className="sign-up-inp-box p-5 text-center" style={{margin: '0% 0% 0% 5%'}}>
                        <FormCard title={{content: 'Sign-up', color: '#ffffff'}} url={this.url} action={this.submit}>
                            ...
                            <Button disabled={true} id="signup-submit" name="Sign Up" type="Submit" onClick={this.unmountInputMonitor}/>
                        </FormCard>
                    </div>
