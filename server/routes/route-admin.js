...
router.put('/upload-picture', async (req, res, next) => {
  ...
  const path_to_admin_pp = path.join(__dirname, '../../front-end/public/images/admin/profile-pics');

  const image = req.files.adminImg;

  fs.readdir( path_to_admin_pp, (err, files) => {
    if( err ) return res.status( 503 ).json({message: `[READDIR]: Something went wrong, please try again`});

    files.forEach( async (file) => {
      await fs.unlink( path.join(path_to_admin_pp, `/${file}`), (err) => {
  ...
  const destination_path = path.join( path_to_admin_pp, image_name );
