...
export default function Signin( props ){
    ...
    const checkAdminExistence = async () => {
        ...
        if( token ){
            axios.get(`http://${window.SERVER_HOST}:${window.SERVER_PORT}/admin/check-status`, {
                headers: {
    ...
    const reqSetAdminSignIn = async ( data ) => { 
        await axios.post(`http://${window.SERVER_HOST}:${window.AUTH_SERVER_PORT}/auth/sign-in`, data)
        .then( res => {
